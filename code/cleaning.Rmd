...
ggplot(data_cleaned, aes(x = cbio_EF)) +
  ...
  labs(title = "Histogram of EF",
       subtitle = "Check for skewness & need for log transformation",
       x = "EF", y = "Count")
#slightly left skewed, trying log transformation
data_cleaned <- data_cleaned %>%
  mutate(
    log_EF = log(cbio_EF)
  )
ggplot(data_cleaned, aes(x = cbio_EF)) +
  geom_histogram(bins = 30)+
  theme_minimal() +
  labs(title = "Histogram of log_EF",
       subtitle = "Check for skewness & need for log transformation",
       x = "log(EF)", y = "Count")
#no change, but appropriate enough to perform tests in analyses
```
Looking at the cbio_LAVmax_BSA variable as the histogram shows partial skewness and an outlier. The outlier, clinically, is not abnormal and it well within the range of a normal cbio_LAVmax_BSA, so I am keeping for now.
```{r}
